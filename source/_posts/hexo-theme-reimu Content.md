---
title: hexo-theme-reimu Content
date: 2024-01-14 21:15:35
urlname: hexo-theme-reimu-content
tags: [Hexo]
categories: [计软杂谈, hexo]
keywords: [hexo, 博客]
description: hexo-theme-reimu Content
excerpt: hexo-theme-reimu Content
mermaid: true
sponsor: false
lang: en
---
...
### Other Elements - abbr, acronym, sub, sup, kbd, etc.
...
$\omega(g(n))=\{f(n):\text{ There exist positive constants }c\text{ and }n_0\text{ such that for all }n \geq n_0\text{, }0\leq cg(n) < f(n)\}$
...
```mermaid
  sequenceDiagram
  participant Client
  participant Proxy Object
  participant Proxy Handler
  participant Target Object
  Client->>Proxy Object: Call Method
  Proxy Object->>Proxy Handler: Forward to Proxy Handler<br>Invoke() Method
  Proxy Handler->>Target Object: Determine Method<br>Call Target Object's Method
  Target Object->>Proxy Handler: Return Result
  Proxy Handler->>Proxy Object: Return Result
  Proxy Object->>Client: Return Result
```

```mermaid
  graph LR
  c1[Client1<br>Socket]--Client Requests Connection-->ss[ServerSocket]
  c2[Client2<br>Socket]--Client Requests Connection-->ss[ServerSocket]
  ss--Server Accepts Request and Creates New Socket-->s1[Socket Created for Client1]
  ss--Server Accepts Request and Creates New Socket-->s2[Socket Created for Client2]
  c1--Dedicated Connection Between Sockets-->s1
  c2--Dedicated Connection Between Sockets-->s2
```
